	...
	case "docker":
		{
			...
			if err != nil {
				return false, errors.Wrap(err, " required debug dependency, docker-compose, is not present or docker is not available. Please run `mesheryctl system check --preflight` to verify system readiness")
			}
